...
// Если нет прогноза на нужную дату — пробуем обновить только один раз!
if (!$forecast) {
    updateWeather($city, $API_KEY, $db);
    $forecast = $weather->getForecast($locationId, $date);
}
...
function updateWeather($city, $API_KEY, $db) {
    ...
    if ($resp === false) {
        error_log("Не удалось получить данные с API для $city");
        return;
    }
    $data = json_decode($resp, true);
    if (!$data || !isset($data['location'])) {
        error_log("Плохой ответ от API для $city: " . $resp);
        return;
    }
